...
class _WritingRestrictionsScreenState extends State<WritingRestrictionsScreen> {
  ...
  Future<void> _loadChildren() async {
    setState(() => _isLoading = true);
    try {
      // ✅ الصحيح: parent_id من SharedPreferences (وليس user.id)
  ...
  Future<void> _loadSettingsForChild(String childId) async {
    ...
    // هذا يخص جهاز الطفل (Accessibility service) — ما نغيره
    await _textMonitorService.setWritingRestrictionsEnabled(isEnabled);
  }
  ...
  Future<void> _saveSettingsForChild(String childId, bool enabled) async {
    ...
    if (enabled && _selectedChildName != null) {
      final token = await _authService.getToken();
      final refreshToken = await _authService.getRefreshToken();
      final parentId = await _authService.getParentId(); // ✅ هذا الصحيح
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            Icon(
              _isAccessibilityEnabled ? Icons.check_circle : Icons.warning_amber_rounded,
              color: _isAccessibilityEnabled ? Colors.green : Colors.orange,
              size: 32,
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    _isAccessibilityEnabled ? 'خدمة المراقبة مفعّلة' : 'خدمة المراقبة غير مفعّلة',
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      color: _isAccessibilityEnabled ? Colors.green.shade700 : Colors.orange.shade700,
                    ),
                  ),
                  Text(
                    _isAccessibilityEnabled
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('اختر الطفل', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(
              value: _selectedChildId,
              decoration: InputDecoration(
              ...
              child: OutlinedButton(
                onPressed: _selectedChildId != null
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => ChildWordRestrictionsScreen(child: selectedChild),
                    ),
                  );
                }
                    : null,
