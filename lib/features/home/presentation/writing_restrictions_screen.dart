...
class _WritingRestrictionsScreenState extends State<WritingRestrictionsScreen> {
  ...
  Future<void> _loadChildren() async {
    ...
    try {
      // ✅ merged: prefer parentId from storage (block-app), fallback to currentUser (main)
      String parentId = (await _authService.getParentId()) ?? '';
      if (parentId.trim().isEmpty) {
        final user = await _authService.getCurrentUser();
        // لو الأب داخل: يكون user.id هو parentId
      ...
      if (parentId.trim().isEmpty) {
        // ما نقدر نجيب أطفال بدون parentId
        if (!mounted) return;
        setState(() {
          _children = [];
      ...
      final response = await _childService.getParentChildren(parentId: parentId);

      if (response.isSuccess && response.data != null) {
        // ✅ merged: فلترة آمنة حسب parentId الحقيقي
        final filtered =
            response.data!.where((c) => c.parentId.toString() == parentId).toList();
        ...
        if (_selectedChildId != null) {
          await _loadSettingsForChild(_selectedChildId!);
        }
      }
    } catch (e) {
      // ignore: avoid_print
  ...
  Future<void> _loadSettingsForChild(String childId) async {
    ...
    // هذا يخص جهاز الطفل (Accessibility service)
  ...
  Future<void> _saveSettingsForChild(String childId, bool enabled) async {
    ...
    // ✅ merged: لا نفقد ميزة حفظ بيانات الطفل للـ Android service
    if (enabled && _selectedChildName != null) {
      final token = await _authService.getToken();
      final refreshToken = await _authService.getRefreshToken();

      // ✅ merged: prefer stored parentId, fallback to resolve from selected child list
      String parentId = (await _authService.getParentId()) ?? '';

      if (parentId.trim().isEmpty) {
        // fallback: حاول تستنتج parentId من الطفل المحدد
        final selected = _children.firstWhere(
          (c) => c.id.toString() == childId,
          orElse: () => Child(id: '', parentId: '', email: '', name: '', age: 0),
        );
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              onChanged: (value) async {
                if (value != null) {
                  final child =
                      _children.firstWhere((c) => c.id.toString() == value);
                  setState(() {
                    _selectedChildId = value;
                    _selectedChildName = child.name;
                  });
                  await _loadSettingsForChild(value);
                }
              },
            ),
            const SizedBox(height: 12),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  const Text('تفعيل مراقبة الكتابة',
                      style:
